...
func (maker *PasetoMaker) CreateToken(userID int64, account_id string, duration time.Duration) (string, *Payload, error) {
	payload, err := NewPayload(userID, account_id, duration)
	if err != nil {
		return "", payload, nil
	}
