// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.24.0
// source: session.sql

package db

import (
	"context"
	"encoding/json"
	"time"

	"github.com/google/uuid"
)

const checkSessionExists = `-- name: CheckSessionExists :one
SELECT EXISTS(SELECT 1 FROM sessions WHERE token = $1 AND scope = $2) AS session_exists
`

type CheckSessionExistsParams struct {
	Token string `json:"token"`
	Scope string `json:"scope"`
}

func (q *Queries) CheckSessionExists(ctx context.Context, arg CheckSessionExistsParams) (bool, error) {
	row := q.db.QueryRowContext(ctx, checkSessionExists, arg.Token, arg.Scope)
	var session_exists bool
	err := row.Scan(&session_exists)
	return session_exists, err
}

const createSession = `-- name: CreateSession :one
INSERT INTO sessions (
  id,
  user_id,
  token,
  scope,
  client_ip,
  payload,
  is_blocked,
  expires_at
) VALUES (
  $1, $2, $3, $4, $5, $6, $7, $8
) RETURNING id, user_id, token, scope, client_ip, payload, is_blocked, expires_at, created_at
`

type CreateSessionParams struct {
	ID        uuid.UUID       `json:"id"`
	UserID    int64           `json:"user_id"`
	Token     string          `json:"token"`
	Scope     string          `json:"scope"`
	ClientIp  string          `json:"client_ip"`
	Payload   json.RawMessage `json:"payload"`
	IsBlocked bool            `json:"is_blocked"`
	ExpiresAt time.Time       `json:"expires_at"`
}

func (q *Queries) CreateSession(ctx context.Context, arg CreateSessionParams) (Session, error) {
	row := q.db.QueryRowContext(ctx, createSession,
		arg.ID,
		arg.UserID,
		arg.Token,
		arg.Scope,
		arg.ClientIp,
		arg.Payload,
		arg.IsBlocked,
		arg.ExpiresAt,
	)
	var i Session
	err := row.Scan(
		&i.ID,
		&i.UserID,
		&i.Token,
		&i.Scope,
		&i.ClientIp,
		&i.Payload,
		&i.IsBlocked,
		&i.ExpiresAt,
		&i.CreatedAt,
	)
	return i, err
}

const deleteExpiredSession = `-- name: DeleteExpiredSession :exec
SELECT delete_expired_sessions()
`

func (q *Queries) DeleteExpiredSession(ctx context.Context) error {
	_, err := q.db.ExecContext(ctx, deleteExpiredSession)
	return err
}

const getSession = `-- name: GetSession :one
SELECT id, user_id, token, scope, client_ip, payload, is_blocked, expires_at, created_at FROM sessions
WHERE id = $1 LIMIT 1
`

func (q *Queries) GetSession(ctx context.Context, id uuid.UUID) (Session, error) {
	row := q.db.QueryRowContext(ctx, getSession, id)
	var i Session
	err := row.Scan(
		&i.ID,
		&i.UserID,
		&i.Token,
		&i.Scope,
		&i.ClientIp,
		&i.Payload,
		&i.IsBlocked,
		&i.ExpiresAt,
		&i.CreatedAt,
	)
	return i, err
}
